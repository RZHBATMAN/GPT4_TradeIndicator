...
Parameterized: caller provides the URL map so each desk can use its own webhook URLs.
"""
...
def send_webhook(signal_data, webhook_urls=None):
    """Send webhook with retry on failure.

    Args:
        signal_data: dict with 'signal' key (e.g. 'TRADE_AGGRESSIVE', 'SKIP')
        webhook_urls: dict mapping signal names to URLs. If None, reads from config
                      (backward compat for desk 1).

    Returns dict with 'success' bool and 'attempts' count.
    """
    if webhook_urls is None:
        # Backward compat: read from config
